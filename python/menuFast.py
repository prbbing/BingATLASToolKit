...
####################################################################################
#                    								   #
# The original script comes from /afs/cern.ch/work/n/nakahama/public/HLTRates/     #
# Courtesy of Yu Nakahama							   #
# This version has been modified to take object index as part of the input         #                 								 
#                								   #
####################################################################################
...
class Threshold:
    def __init__(self,thresh):
        ...
        if thresh[0].isdigit():
            ...
            thresh=thresh[1:]
        for v in thresh.split("-")[0]:
            if v.isdigit():
                value+=v
            else:
                var+=v
        for v in thresh.split("-")[1]:
            index.append(v)
        ...
        if Vthresh:
            ...
            self.cut='%s_pt[%d]>%d' % ( self.var,int(index[self.mult-1]),self.thresh)
        if var=='HT':
